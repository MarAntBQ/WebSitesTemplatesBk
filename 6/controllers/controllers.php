        ...
        if(isset($_POST["controller-action"])) {
           $controller_action = $_POST["controller-action"];
            if ($controller_action == "SendMessage") {
                ...
                switch ($way) {
                    case "General-Support":
                        $output = "home#contact";
                        $outputmail = "EMAIL";
                        $outputmailName = "Marbust Support Center";
                        $outputtittle = "Formulario de Contacto | Marbust Sites MBTC®";
            ...
            $mail->Password = "PASSWORD";					// Enter the email password created through cPanel
                ...
                if(!$mail->Send())
                {
                    $_SESSION["MailerAnswer"] = '<p class="tac fs18px lh2em mb15px tbold">Lo sentimos pero el Mensaje no pudo ser enviado</p>';
                    header("Location: $output");    
                    exit;
                }
                
                $_SESSION["MailerAnswer"] = '<p class="tac fs18px lh2em mb15px tbold">Gracias '.$name.', tu mensaje ha sido enviado correctamente.</p>';
                    header("Location: $output");    
                    exit;                   
            }
        }
